...
const MenuBar: React.FC = () => {
  ...
  useEffect(() => {
    // Use handler pattern to satisfy lint rules
    const updateTime = () => setTime(new Date());
    updateTime(); // Initial call via handler
  ...
  return (
    <div className="h-7 bg-black/40 backdrop-blur-xl border-b border-white/10 flex items-center px-4 text-white text-xs">
      ...
      <div className="ml-auto flex items-center gap-3">
        ...
        <span>{time ? time.toLocaleTimeString('en-US', { hour: '2-digit', minute: '2-digit' }) : '--:--'}</span>
        <span>{time ? time.toLocaleDateString('en-US', { month: 'short', day: 'numeric' }) : '---'}</span>
      </div>
    </div>
  );
};
